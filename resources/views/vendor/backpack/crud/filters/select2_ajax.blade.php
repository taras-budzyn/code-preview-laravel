...
<li filter-name="{{ $filter->name }}" filter-type="{{ $filter->type }}" class="nav-item dropdown {{ Request::get($filter->name)?'active':'' }}">
	<a href="#" class="nav-link dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">{{ $filter->label }} <span class="caret"></span></a>
	<div class="dropdown-menu p-0 ajax-select">
		<div class="form-group mb-0">
			<select id="filter_{{ $filter->name }}" name="filter_{{ $filter->name }}" class="form-control input-sm select2" data-filter-type="select2_ajax" data-filter-name="{{ $filter->name }}" data-select-key="{{ isset($filter->options['select_key']) ? $filter->options['select_key'] : 'id' }}" data-select-attribute="{{ isset($filter->options['select_attribute']) ? $filter->options['select_attribute'] : 'name' }}" placeholder="{{ $filter->placeholder }}" filter-minimum-input-length="{{ isset($filter->options['minimum_input_length']) ? $filter->options['minimum_input_length'] : 2 }}" filter-method="{{ isset($filter->options['method']) ? $filter->options['method'] : 'GET' }}">
